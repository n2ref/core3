...
use Core3\Mod\Admin\Alert;
use Core3\Mod\Admin\Common;
use Core3\Mod\Admin\Email;
use Core3\Mod\Admin\Exception;
use Core3\Mod\Admin\InstallModule;
use Core3\Mod\Admin\LdapAuth;
use Core3\Mod\Admin\tabs;
use Core3\Mod\Admin\Templater2;
use Core3\Mod\Admin\Zend_Cache;
use Core3\Mod\Admin\Zend_Db_Expr;
use Core3\Mod\Admin\Zend_Registry;
use Core3\Mod\Admin\Zend_Session_Namespace;
...
class Mobile extends Common {
	...
	public function action_index() {
        ...
        try {
            ...
            if (empty($changedMods)) {
                echo '<h3>' . $this->_("Система работает в штатном режиме.") . '</h3>';
            } else {
                echo '<h3 style="color:red;">' . $this->_("Обнаружены изменения в файлах модулей:") . ' ' . implode(", ", $changedMods) . '</h3>';
            }
        } catch (Exception $e) {
            $install->addNotice($this->_("Аудит файлов модулей"), $e->getMessage(), $this->_("Ошибка"), "danger");
        }
	...
	public function action_switch(){
		try {
			if (!isset($_POST['data'])) {
				throw new Exception($this->_('Произошла ошибка! Не удалось получить данные'));
			}
    ...
    public function action_delete(Array $params)
    {
        ...
        $resource   = $params['res'];
        if (!$resource) throw new Exception($this->_("Не удалось определить идентификатор ресурса"), 13);
        if (!$params['id']) throw new Exception($this->_("Нет данных для удаления"), 13);
        $ids        = explode(",", $params['id']);
		$sessData = $sess->$resource;
        $deleteKey  = $sessData['deleteKey'];
        if (!$deleteKey) throw new Exception($this->_("Не удалось определить параметры удаления"), 13);
        [$table, $refid] = explode(".", $deleteKey);
        if (!$table || !$refid) throw new Exception($this->_("Не удалось определить параметры удаления"), 13);
        ...
        if (($this->checkAcl($resource, 'delete_all') || $this->checkAcl($resource, 'delete_owner'))) {
            ...
            try {
                ...
                if ($authorOnly) {
                    if ($noauthor) {
                        throw new Exception($this->_("Данные не содержат признака автора!"));
                    } else {
                ...
                if ($nodelete) {
                    foreach ($ids as $key) {
                        ...
                        if ($authorOnly) {
                            $where[] = $this->db->quoteInto("author = ?", $auth->LOGIN);
                        }
	...
	public function action_users () {
		if (!$this->auth->ADMIN) throw new Exception(911);
		//require_once 'core3/mod/ModAjax.php';
		...
		if (!empty($_POST['sendSupportForm'])) {
			...
			try {
				if (empty($supportFormModule)) {
					throw new Exception($this->_('Выберите модуль.'));
				}
				if (empty($supportFormMessage)) {
					throw new Exception($this->_('Введите текст сообщения.'));
				}
				...
				if ($dataUser) {
                    ...
                    if (isset($result['error'])) {
                        throw new Exception($this->_('Не удалось отправить сообщение'));
                    } else {
	...
	public function feedbackForm() {
		...
		$this->printJs("core3/mod/admin/feedback.js", true);
		require_once 'classes/Templater2.php';
		$tpl = new Templater2("core3/mod/admin/html/feedback.tpl");
	...
	public function userProfile() {
		...
		if ($this->auth->LOGIN !== 'root' && $this->auth->LDAP) {
			require_once 'core3/inc/classes/LdapAuth.php';
			$ldap = new LdapAuth();
			$ldap->getLdapInfo($this->auth->LOGIN);
		}
		...
		if (!empty($name)) {
			...
			$name = $this->auth->LOGIN;
		}
		if (!empty($name)) $name = '<b>' . $name . '</b>';
		$out = '<div>' . sprintf($this->_("Здравствуйте, %s"), $name) . '</div>';
		...
		if ($res) {
			$out .= '<div>' . sprintf($this->_("Последний раз Вы заходили %s с IP адреса %s"), '<b>' . $res['login_time2'] . '</b>', '<b>' . $res['ip'] . '</b>') . '</div>';
		}
	...
	public function action_enum() {
		if (!$this->auth->ADMIN) throw new Exception(911);
		$this->printJs("core3/mod/admin/enum.js");
    ...
    public function checkModulesChanges($install) {
        ...
        if (!$server) {
            $install->addNotice("", $this->_("Не задан параметр 'host' в conf.ini"), $this->_("Отправка уведомлений отключена"), "info2");
        }
                ...
                if ($admin_email && $server) {
                    ...
                    if (!$is_send) {
                        ...
                        if (isset($answer['error'])) {
                            $install->addNotice("", $answer['error'], $this->_("Уведомление не отправлено"), "danger");
                        }
                    }
                }
            }
