 ...
 */
class Modules_Install extends Common {
    ...
    private function checkXml() {
        if (!is_file($this->tempDir . "/install/install.xml")) {
            throw new Exception($this->_("install.xml не найден. Установка прервана."));
        }
    }
...
//        }
    ...
    private function getAccess() {
        ...
        $access_add = array();
            if (!empty($Inf['install']['access']['default'])) {
                ...
                $this->addNotice($this->_("Права доступа"), "Права по умолчанию", "Добавлены", "info");
            } else {
                $this->addNotice($this->_("Права доступа"), "Права по умолчанию", "Отсутствуют", "warning");
            }
    ...
    private function getSubModules($m_id) {
        ...
        if (!empty($Inf['install']['submodules']['sm'])) {
            if (!empty($Inf['install']['submodules']['sm']['id'])) {
            ...
            foreach ($Inf['install']['submodules']['sm'] as $valsub) {
                $access = array();
                $access_add = array();
                if (!empty($valsub['id']) && !empty($valsub['sm_name'])) {
    ...
    public function Install() {
        ...
        if (!empty($subModules)) {
            ...
            {
                $this->db->insert('core_modules_actions', $subval);
            }
    ...
    public function migrateSql() {
        ...
        if (!empty($file_name)) {
            ...
            if (!empty($file_name) && is_file($file_loc)) {
                ...
                throw new Exception($this->_("Не найден файл $file_name для обновления модуля!"));
            }
        }
        if (!$sql) {
            return false;
            ...
            $sql = $this->SQLPrepareToExecute($sql);//готовим
            if (!$this->checkSQL($sql)) {
                throw new Exception($this->_("Попытка удаления таблиц не относящихся к модулю!"));
            }
            //разбиваем запросы на отдельные
            $sql = $this->SQLToQueriesArray($sql);
            foreach ($sql as $qu) {
            ...
            $this->addNotice($this->_("Таблицы модуля"), $this->_("Таблицы добавлены"), "Успешно", "info");
        }
        return true;
    }
    ...
    private function Upgrate() {
        ...
        $this->db->query("DELETE FROM `core_modules_actions` WHERE m_id = {$m_id}");
        ...
        if ($this->is_visible == "N") {
            $msg = !empty($this->module_is_off) ? (" вклчючите '" . implode("','", $this->module_is_off) . "', а потом этот модуль") : " включите модуль";
            $this->addNotice($this->_("Обновление"), $this->_("Обновление завершено"), "Для работы{$msg}", "warning");
        } else {
            $this->addNotice($this->_("Обновление"), $this->_("Обновление завершено"), "Успешно", "info");
        }
    }
...
//            $this->addNotice("Файлы модуля", implode("<br>", $this->deleteFilesInfo['is_not_writeable']), "Папка закрыта для записи, удалите её самостоятельно", "danger");
            $this->addNotice($this->_("Файлы модуля"), $this->_("Удаление"), "Папка закрыта для записи, удалите её самостоятельно", "danger");
...
//            $this->addNotice("Файлы модуля", implode("<br>", $this->deleteFilesInfo['not_exists']), "Не существует", "info");
            $this->addNotice($this->_("Файлы модуля"), $this->_("Удаление"), "Файлы не найдены", "info");
...
//            $this->addNotice("Файлы модуля", implode("<br>", $this->deleteFilesInfo['success']), "Файлы удалены", "info");
            $this->addNotice($this->_("Файлы модуля"), $this->_("Удаление"), "Успешно", "info");
        }
    }
    ...
    public function downloadModTemplate($template) {
        try {
            ...
            $template_path = "core3/mod_tpl/" . $template;
            ...
            if ($res === TRUE) {
                ...
                throw new Exception($this->_("Ошибка создания архива"));
            }
        }
        catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function getUninstallSQL() {
        ...
        if (!empty($sql)) {
            ...
            if ($this->checkSQL($sql)) {
                ...
                $this->addNotice($this->_("Таблицы модуля"), $this->_("В SQL для удаления модуля обнаружена попытка удаления таблиц не относящихся к модулю"), "SQL проигнорирован", "warning");
                return null;
            }
        }
        return null;
    ...
    public function printNotices($tab = false){
        ...
        if ($tab) {
            $html .= "<br><input type=\"button\" class=\"button\" value=\"" . $this->_('Вернуться к списку модулей') . "\" onclick=\"load('index.php?module=admin&action=modules&tab_mod={$tab}');\">";
        }
        return $html;
    }
    ...
    public function checkVer()
    {
        ...
        if ($this->curVer == $this->mInfo['install']['version']) {
            throw new Exception($this->_("У вас уже установлена эта версия!"));
        } elseif ($this->curVer > $this->mInfo['install']['version']) {
            throw new Exception($this->_("У вас стоит более актуальная версия!"));
        }
    ...
    public function copyFiles($dir, $dirTo) {
        ...
        if ( ! empty($this->copyFilesInfo['error'])) {
            $this->addNotice($this->_("Файлы модуля"), $this->_("Копирование"), $this->_("Файлы не скопированы, скопируйте их вручную"), "danger");
        } elseif ( ! empty($this->copyFilesInfo['success'])) {
            $this->addNotice($this->_("Файлы модуля"), $this->_("Копирование"), $this->_("Файлы скопированы успешно"), "info");
        }
    }
    ...
    private function prepareToInstall() {
        ...
        if (!empty($compare)) {
            throw new Exception($this->_("Хэши файлов модуля не совпадают с эталоном! Установка прервана."));
        }
        ...
        $this->installPath 	= ((strtolower($mInfo['install']['module_system']) == "y" ? "core3/" : "") . "mod/{$mInfo['install']['module_id']}/v{$mInfo['install']['version']}");
    ...
    public function mInstall($mod_id){
        ...
        try {
            ...
            if (empty($temp)) {
                throw new Exception($this->_('Модуль не найден в доступных модулях'));
            }
            ...
            $this->db->rollBack();
            $this->addNotice($this->_("Установщик"), $this->_("Установка прервана, произведен откат транзакции"), "Ошибка: {$e->getMessage()}", "danger");
            return $st . $this->printNotices(2);
        }
    }
    ...
    public function mInstallFromRepo($repo_url, $m_id){
        ...
        try {
            ...
            //TODO вести лог
            $this->addNotice($this->_("Установщик"), $this->_("Установка прервана, произведен откат транзакции"), "Ошибка: {$msg}", "danger");
        }
        return $st . $this->printNotices(2);
    }
...
//                    $copy_list[$key]['install_info'] = "<img src=\"core3/html/".THEME."/img/box_out_disable.png\" title=\"Уже установлен\" border=\"0\"/></a>";
                } elseif (!empty($deps)) {
//                    $copy_list[$key]['install_info'] = "<img onclick=\"alert('Сначала установите модули: " . implode(", ", $needToInstall) . "')\" src=\"core3/html/".THEME."/img/box_out.png\" border=\"0\" title=\"Установить\"/>";
                    $copy_list[$key]['install_info'] = "<img src=\"core3/html/".THEME."/img/box_out_disable.png\" title=\"Требуется установка дополнительных модулей\" border=\"0\"/>";
                } else {
                    $copy_list[$key]['install_info'] = "<img onclick=\"modules.requestToRepo('$mName', 'v$mVersion', '{$copy_list[$key]['id']}', '{$this->repo_url}', 'install');\" src=\"core3/html/".THEME."/img/box_out.png\" border=\"0\" title=\"Установить\"/>";
                }
        ...
        try {
            ...
            if (!empty($mInfo)) {
                ...
                if ($is_used_by_other_modules === false) {
                        ...
                        if ($this->checkSQL($sql)) {
                            ...
                            foreach ($sql as $qu) {
                                ...
                                try {
                                    ...
                                    $this->addNotice($this->_("Таблицы модуля"), "Ошибка при удалении таблиц (удаление продолжается)", $e->getMessage(), "warning");
                                }
                            }
                            $this->addNotice($this->_("Таблицы модуля"), $this->_("Удаление таблиц"), $this->_("Выполнение SQL завершено"), "info");
                        } else {
                            $this->addNotice($this->_("Таблицы модуля"), $this->_("Таблицы не удалены"), "Попытка удаления таблиц не относящихся к модулю, удалите их самостоятельно!", "warning");
                        }
                    } else {
                        $this->addNotice($this->_("Таблицы модуля"), $this->_("Таблицы не удалены"), "Инструкции по удалению не найдены, удалите их самостоятельно!", "warning");
                    }
                    //удаляем субмодули
                    $this->db->delete("core_modules_actions", $this->db->quoteInto("m_id =?", $m_id));
                    $this->addNotice($this->_("Субмодули"), $this->_("Удаление субмодулей"), $this->_("Выполнено"), "info");
                    //удаляем регистрацию модуля
                    $this->db->delete('core_modules', $this->db->quoteInto("module_id=?", $mInfo['module_id']));
                    $this->addNotice($this->_("Регистрация модуля"), $this->_("Удаление сведений о модуле"), $this->_("Выполнено"), "info");
                    ...
                    //удаляем файлы
                    $modulePath = (strtolower($mInfo['is_system']) == "y" ? "core3/" : "") .  "mod/{$mInfo['module_id']}/v{$mInfo['version']}";
                    if (strtolower($mInfo['is_system']) == 'n') {
                        $this->deleteFolder($modulePath);
                    } else {
//                        $this->deleteFolder($modulePath);
                        $this->addNotice($this->_("Файлы модуля"), $this->_("Файлы не удалены"), $this->_("Файлы системных модулей удаляются вручную!"), "warning");
                    }
                ...
                $this->addNotice($this->_("Деинсталяция"), "Статус", "Завершена", "info");
                return "<h3>Деинсталяция модуля " . (!empty($mInfo['m_name']) ? "'{$mInfo['m_name']}'" : "") . "</h3>" . $this->printNotices(1);

            } else{//если модуль не существует
                throw new Exception($this->_("Модуль уже удален или не существует!"));
            }

        } catch (Exception $e) {
            $this->addNotice($this->_("Деинсталяция"), "Ошибка: {$e->getMessage()}", $this->_("Деинсталяция прервана"), "danger");
            return "<h3>Деинсталяция модуля " . (!empty($mInfo['m_name']) ? "'{$mInfo['m_name']}'" : "") . "</h3>" . $this->printNotices(1);
        }
    }
...
//                }
    ...
    public function getInfoAllAvailMods() {
        ...
        $modules = $this->db->fetchAll("
            SELECT id, 
                   install_info 
            FROM core_modules_available
        ");

        foreach ($modules as $module) {
            $info = unserialize($module['install_info']);
            if ( ! empty($info['install']) &&
                isset($info['install']['name']) &&
                isset($info['install']['version'])
            ) {
                $info['location']    = 'avail';
                $info['location_id'] = $module['id'];
                $list[$info['install']['name']][$info['install']['version']] = $info;
            }
        }
        ...
        if (!empty($mod_repos)){
            ...
            try {
                //готовим аякс запросы к репозиториям
                foreach($mod_repos as $info => $repo_url){
                    $repo_url = trim($repo_url);
                    if (!empty($repo_url) && substr_count($repo_url, "repo?apikey=") != 0) {
                        ...
                        foreach ($repo_list as $m_id=>$info) {
                            if (empty($list[$info['install']['module_id']][$info['install']['version']])) {
                                $info['location']      = 'repo';
                                $info['location_id']   = $m_id;
                                $info['location_url']  = $repo_url;
                                $list[$info['install']['module_id']][$info['install']['version']] = $info;
                            }
                        }
                    }
                }
    ...
    public function checkModUpdates($mod_id) {
        ...
        try {
            ...
            if (!empty($data) && !empty($files_hash)) {
                ...
                $this->addNotice($this->_("Поиск обновлений"), $this->_("Поиск в доступных модулях и репозиториях"), $this->_("Обновления не найдены"), "warning");
            }
            ...
            //TODO вести лог
            $this->addNotice($this->_("Обновление"), $this->_("Обновление прервано, произведен откат транзакции"), "Ошибка: {$msg}", "danger");
        }
    ...
    public function checkInstalledModsUpdates() {
        ...
        $updates           = [];
        $modules_available = $this->getInfoAllAvailMods();
        $modules_all       = $this->db->fetchAll("
            SELECT name, 
                   version 
            FROM core_modules
        ");
        if ( ! empty($modules_all)) {
            foreach ($modules_all as $module) {
                if ( ! empty($modules_available[$module['name']])) {
                    foreach ($modules_available[$module['name']] as $available_version => $available_module) {
                        if ($available_version > $module['version'] &&
                            isset($available_module['migrate']["v{$module['version']}"]) &&
                            (empty($updates[$module['name']]) || $available_version > $updates[$module['name']]['version'])
                        ) {
                            $updates[$module['module_id']] = array(
                                'name'    => $module['name'],
                                'version' => $available_version,
                                'm_name'  => $available_module['install']['module_name']
                            );
                        }
                    }
                }
